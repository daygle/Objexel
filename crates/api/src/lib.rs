...
async fn download_model(State(state): State<Arc<AppState>>, Path(id): Path<String>) -> Result<(StatusCode, Json<ModelDownload>), ErrorResponse> {
    ...
    tokio::spawn(async move {
        ...
        match result { Ok(path) => { let input = CreateModel { name: entry.name, version: entry.version, model_type: entry.model_type.clone(), path: path.to_string_lossy().into_owned(), input_width: entry.input_width, input_height: entry.input_height, class_list: if entry.class_list.is_empty() && matches!(entry.model_type.as_str(), "coco" | "yolo-coco") { coco80_labels() } else { entry.class_list }, enabled: false, default_model: false }; match database_clone.create_model(input).await { Ok(model) => { let _=database_clone.update_model_download(download.id,"completed",100,0,None,None,Some(model.id)).await; }, Err(error) => { let _=database_clone.update_model_download(download.id,"failed",0,0,None,Some(&error.to_string()),None).await; } } }, Err(error) => { let message=error.to_string(); let _=database_clone.update_model_download(download.id,"failed",0,0,None,Some(&message),None).await; } }
    });
