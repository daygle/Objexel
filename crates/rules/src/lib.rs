...
impl RuleEngine {
    ...
    fn matches(&self, rule: &Rule, context: &ObservationContext<'_>) -> bool {
        !rule.conditions.is_empty() && rule.conditions.iter().all(|condition|
            ...
            && condition.behaviour_type.as_deref().map(|value| context.behaviour_type == Some(value)).unwrap_or(true)
...
mod tests {
    ...
    fn matching_rule_is_suppressed_until_cooldown() {
        ...
        let context = ObservationContext { observation: &observation, object_class: Some("cat"), zone_id: None, confidence: Some(0.9), duration_ms: None, behaviour_type: None, identity_id: None, familiarity: None, priority: None, anomaly_score: None };
