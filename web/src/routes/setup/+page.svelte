<script lang="ts">
  ...
  let username = ''; let email = ''; let password = ''; let message = ''; let busy = false;
  async function submit() { busy = true; const response = await api('/api/auth/setup', { method:'POST', json:{ username, email: email || null, password, role:'administrator' }, redirectOnUnauthorized:false }); if (response.ok) { const result = await response.json(); setCsrf(result.csrf_token); await goto('/observations'); } else message = await apiError(response); busy = false; }
</script>
