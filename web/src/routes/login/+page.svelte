<script lang="ts">
  ...
  async function submit() { busy = true; message = ''; const response = await api('/api/auth/login', { method:'POST', json:{ username, password }, redirectOnUnauthorized:false }); if (response.ok) { const result = await response.json(); setCsrf(result.csrf_token); await goto('/observations'); } else message = await apiError(response); busy = false; }
</script>
