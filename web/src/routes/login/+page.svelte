<script lang="ts">
  ...
  onMount(async () => { const response = await api('/api/auth/setup', { redirectOnUnauthorized:false }); if (response.ok) { const data = await response.json(); setupNeeded = data.setup_required; } });
